    ...
    if (isLoggedIn()) {
        ...
        userProfileDiv.innerHTML = `
            ...
            <div class="user-profile-dropdown">
                <a href="#" id="viewProfileBtnSidebar">عرض الملف الشخصي</a>
                <a href="#" id="logoutBtnSidebar">تسجيل الخروج</a>
            </div>
        `;
...
function checkGoogleLoginCallback() {
    ...
    if (url.includes('google_id=')) {
        ...
        console.log("Parsed URL Parameters:", Object.fromEntries(params));
        console.log("Attempting to retrieve user details from callback:");
        console.log("Callback - Google ID:", googleId);
        console.log("Callback - Name:", name);
        console.log("Callback - Email:", email);
        console.log("Callback - Picture URL:", picture);

        if (googleId) {
            localStorage.setItem('google_id', googleId);
            console.log("Stored google_id to localStorage.");
        } else {
            console.warn("Google ID not found in callback parameters.");
        }

        if (name) {
            localStorage.setItem('name', name);
            console.log("Stored name to localStorage:", name);
        } else {
            console.warn("Name not found or empty in callback parameters. User profile name might be default.");
        }

        if (email) {
            localStorage.setItem('email', email);
            console.log("Stored email to localStorage:", email);
        } else {
            console.warn("Email not found or empty in callback parameters. User profile email might be default.");
        }

        if (picture) {
            localStorage.setItem('picture', picture);
            console.log("Stored picture to localStorage:", picture);
        } else {
            console.warn("Picture URL not found or empty in callback parameters. User profile picture might be default/letter avatar.");
        }
...
function populateUserProfilePage() {
    ...
    const userName = localStorage.getItem('name') || 'مستخدم';
    const userEmail = localStorage.getItem('email') || 'غير متاح';
    const userPicture = localStorage.getItem('picture');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function addConversationToHistory(messages, titlePrefix = "محادثة") {
    ...
    function appendMessagesToCurrentChatOrStartNew(newMessages, titlePrefixIfNew = "محادثة") {
    ...
    function handleUseCaseSelection(caseKey) {
        ...
        switch (caseKey) {
            case 'general':
                aiWelcomeMessage = "नमस्ते! मैं आपका मुख्य सहायक हूं. मैं आपकी कैसे मदद कर सकता हूँ?";
                suggestions = [
                    { title: 'اكتب قصيدة عن الربيع', prompt: 'اكتب قصيدة عن الربيع' },
                    { title: 'اشرح الحوسبة الكمومية بعبارات بسيطة', prompt: 'اشرح الحوسبة الكمومية بعبارات بسيطة' },
                    { title: 'لخص النقاط الرئيسية في هذا المقال...', prompt: 'لخص النقاط الرئيسية في هذا المقال: [الصق المقال هنا]' }
                ];
                break;
            case 'education':
                aiWelcomeMessage = "नमस्ते! मैं आपका शैक्षणिक सहायक हूं। क्या आपके पास अपनी पढ़ाई के बारे में कोई प्रश्न हैं?";
                suggestions = [
                    { title: 'اشرح نظرية النسبية لأينشتاين', prompt: 'اشرح نظرية النسبية لأينشتاين بطريقة مبسطة.' },
                    { title: 'ما هي الدورة المائية؟', prompt: 'ما هي الدورة المائية وكيف تعمل؟' },
                    { title: 'ساعدني في حل هذه المسألة الرياضية: 2x + 5 = 15', prompt: 'ساعدني في حل هذه المسألة الرياضية: 2x + 5 = 15، مع شرح الخطوات.' }
                ];
                break;
            case 'translation':
                aiWelcomeMessage = "नमस्ते! मैं आपका अनुवाद सहायक हूं. आज मैं आपकी किस भाषा में मदद कर सकता हूँ?";
                suggestions = [
                    { title: 'ترجم "مرحباً أيها العالم!" إلى الإسبانية', prompt: 'ترجم "Hello, World!" إلى الإسبانية' },
                    { title: 'ما معنى "déjà vu" بالإنجليزية؟', prompt: 'ما معنى "déjà vu" بالإنجليزية؟' },
                    { title: 'ترجم هذا النص من الفرنسية إلى العربية: [الصق النص هنا]', prompt: 'ترجم هذا النص من الفرنسية إلى العربية: ' }
                ];
                break;
            case 'data_analysis':
                aiWelcomeMessage = "नमस्ते! मैं आपका डेटा विश्लेषण सहायक हूं। कृपया डेटा प्रदान करें या अपनी विश्लेषण आवश्यकताओं का वर्णन करें।";
                suggestions = [
                    { title: 'حلل اتجاهات المبيعات لهذه البيانات...', prompt: 'لدي مجموعة بيانات مبيعات بالتنسيق التالي (أعمدة: التاريخ، المنتج، الكمية، السعر). هل يمكنك مساعدتي في تحديد الاتجاهات الرئيسية؟ [الصق عينة من البيانات أو صفها]' },
                    { title: 'ما هي أفضل طريقة لتصور هذه البيانات؟', prompt: 'ما هي أفضل طريقة لتصور بيانات توضح العلاقة بين الإنفاق الإعلاني وحركة المرور على الموقع؟' },
                    { title: 'أنشئ ملخصاً لهذه الأرقام المالية.', prompt: 'أنشئ ملخصاً لهذه الأرقام المالية: الإيرادات: 150 ألف دولار، التكاليف: 90 ألف دولار، الربح: 60 ألف دولار.' }
                ];
                break;
            case 'screenwriting':
                aiWelcomeMessage = "नमस्ते! मैं आपका लेखन साझेदार हूं। क्या आपके पास कोई रोमांचक कहानी का विचार है?";
                suggestions = [
                    { title: 'اقترح حبكة لقصة قصيرة خيال علمي', prompt: 'اقترح حبكة لقصة قصيرة خيال علمي تدور أحداثها في مستعمرة على المريخ.' },
                    { title: 'اكتب حوارًا بين محقق وشاهد.', prompt: 'اكتب حوارًا قصيرًا بين محقق صارم وشاهد عصبي في جريمة قتل.' },
                    { title: 'طور شخصية لبطل خارق جديد.', prompt: 'طور شخصية لبطل خارق جديد تكون قوته التحكم في الطقس، مع خلفية مأساوية.' }
                ];
                break;
            case 'media_ops':
                aiWelcomeMessage = "नमस्ते! मैं आपका मीडिया ऑपरेशन सलाहकार हूं। क्या आपको सामग्री रणनीति या प्रचार विचारों की आवश्यकता है?";
                suggestions = [
                    { title: 'ابتكر شعارًا تسويقيًا لمقهى جديد.', prompt: 'ابتكر 3 شعارات تسويقية جذابة لمقهى جديد متخصص في القهوة العضوية.' },
                    { title: 'اكتب منشورًا على وسائل التواصل الاجتماعي للترويج لمنتج جديد.', prompt: 'اكتب منشورًا على Instagram للترويج لسماعات رأس لاسلكية جديدة مقاومة للضوضاء.' },
                    { title: 'ضع استراتيجية محتوى لمدونة سفر.', prompt: 'ضع استراتيجية محتوى لمدة شهر لمدونة سفر تستهدف محبي المغامرات ذوي الميزانية المحدودة.' }
                ];
                break;
            case 'image_parsing':
                aiWelcomeMessage = "नमस्ते! मैं इमेज पार्सिंग ए.आई. हूं। कृपया एक चित्र अपलोड करें, और मैं इसकी विषय-वस्तु को समझने का प्रयास करूंगा।";
                suggestions = [
                    { title: 'صف هذه الصورة بالتفصيل.', prompt: 'صف هذه الصورة بالتفصيل.' },
                    { title: 'ما هو الكائن الرئيسي في هذه الصورة؟', prompt: 'ما هو الكائن الرئيسي في هذه الصورة؟' },
                    { title: 'هل يمكنك التعرف على النص في هذه الصورة؟', prompt: 'هل يمكنك التعرف على النص في هذه الصورة؟' }
                ];
                break;
            case 'ai_drawing':
                aiWelcomeMessage = "नमस्ते! मैं एआई ड्राइंग सहायक हूं। आप क्या चित्रित करना चाहेंगे? अपने विचार का वर्णन करें!";
                suggestions = [
                    { title: 'أنشئ منظرًا طبيعيًا خياليًا.', prompt: 'أنشئ منظرًا طبيعيًا خياليًا مع جبال عائمة وشلالات متوهجة.' },
                    { title: 'ارسم قطة ترتدي قبعة رعاة البقر.', prompt: 'ارسم قطة لطيفة ترتدي قبعة رعاة البقر وتجلس على السياج.' },
                    { title: 'صمم شعارًا لشركة تكنولوجيا.', prompt: 'صمم شعارًا حديثًا وبسيطًا لشركة تكنولوجيا ناشئة تركز على الذكاء الاصطناعي.' }
                ];
                break;
            case 'image_to_image':
                aiWelcomeMessage = "नमस्ते! मैं इमेज-टू-इमेज एआई हूं। कृपया एक छवि अपलोड करें और मुझे बताएं कि आप इसे किस प्रकार बदलना चाहेंगे।";
                suggestions = [
                    { title: 'حول هذه الصورة إلى لوحة زيتية.', prompt: 'حول هذه الصورة إلى لوحة زيتية بأسلوب فان جوخ.' },
                    { title: 'اجعل هذه الصورة تبدو كأنها من فيلم خيال علمي قديم.', prompt: 'اجعل هذه الصورة تبدو كأنها من فيلم خيال علمي من السبعينيات.' },
                    { title: 'غير ألوان هذه الصورة إلى درجات البني الداكن.', prompt: 'غير ألوان هذه الصورة إلى درجات البني الداكن (سيبيا).' }
                ];
                break;
            default:
                aiWelcomeMessage = "नमस्ते! कृपया कोई परिदृश्य चुनें या मुझसे सीधे प्रश्न पूछें।";
        ...
        if (promptSuggestionsArea) {
            ...
            suggestions.forEach(text => {
                ...
                suggBtn.textContent = text.title;
                suggBtn.onclick = () => {
                    chatInput.value = text.prompt;
                    chatInput.focus();
    ...
    function animateReviewCardsLoop() {
        ...
        setTimeout(() => {
            // allReviewCards.forEach(card => {
            //     // card.style.transition = 'opacity 0.5s cubic-bezier(0.68, -0.55, 0.27, 1.55)'; // Re-enable transition
            // });
    ...
    function loadChatHistory() {
        ...
        function populateHistory(listElement) {
            ...
            if (history.length === 0) {
                ...
                noHistoryItem.textContent = 'لم يتم العثور على محادثات سابقة.';
            ...
            history.slice().reverse().forEach(chat => { // Display newest first
                ...
                renameBtn.addEventListener('click', () => {
                    const newTitle = prompt('أدخل عنوانًا جديدًا للمحادثة:', chat.title);
                ...
                deleteBtn.addEventListener('click', () => {
                    if (confirm(`هل أنت متأكد أنك تريد حذف "${chat.title}"?`)) {
    ...
    function loadConversation(chatId) {
        ...
        if (!conversation) {
            addMessageToChat("خطأ: تعذر تحميل المحادثة.", "bot");
            return;
        }
    ...
    function deleteConversation(chatId) {
        if (confirm('هل أنت متأكد أنك تريد حذف هذه المحادثة؟')) {
            try {
                let history = getChatHistory();
                history = history.filter(conv => conv.id !== chatId);
                saveChatHistory(history);
                loadChatHistory(); 
    
                if (currentLoadedChatId === chatId && messageArea) {
                    messageArea.innerHTML = '';
                    addMessageToChat('بدأت محادثة جديدة', 'bot'); 
                    currentLoadedChatId = null;
                }
            } catch (error) {
                console.error('خطأ في حذف المحادثة:', error);
                if(messageArea) addMessageToChat(`خطأ في حذف المحادثة: ${error.message}`, 'bot');
                else alert(`خطأ في حذف المحادثة: ${error.message}`);
            }
        }
    }
    ...
    function renameConversation(chatId, newTitle) {
        let history = getChatHistory();
        const conversation = history.find(conv => conv.id === chatId);
        if (conversation) {
    ...
    if (window.location.pathname === '/index.html' || window.location.pathname === '/') {
        ...
        const requireLoginAndRedirect = (event, elementToDisableTemporarily = null) => {
            if (!isLoggedIn()) {
                ...
                alert('يرجى تسجيل الدخول لاستخدام الميزة.');
